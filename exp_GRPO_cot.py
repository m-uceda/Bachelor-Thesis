...
def main():
    ...
    train_json_file = "cot_train_data.json" # Change to name corresponding to the dataset you want to use
    test_json_file = "cot_test_data.json"
    train_json_dataset = get_data(train_json_file, COT_PROMPT, TRAIN_JSON_FILE)
    dev_json_dataset = get_data(test_json_file, COT_PROMPT, DEV_JSON_FILE)
    ...
    methods = ["cot"] # Options: "direct", "cot", "prolog", "prolog_fix", "prolog_retry", "prolog_backup"
    train_datasets = {"cot": train_dataset}	
    test_datasets = {"cot": test_dataset}
    base_methods = {"cot": "cot"}
    answer_types = ["cot_answer"]
    experiment_name = "exp_GRPO_cot"
    rewards = {"cot": [cot_correctness_reward_func, cot_letter_reward_func, cot_format_reward_func]}
